...
test("new system prompt is present after keep_context mode switch", async ({ page, agentType }) => {
  ...
  await sendMessage(page, "call task test_sysprompt_mode_a call switchmode check_new");
...
test("old system prompt is absent after keep_context mode switch", async ({ page, agentType }) => {
  await enterSession(page);
  ...
  await sendMessage(page, "call task test_sysprompt_mode_a call switchmode check_old_absent");
