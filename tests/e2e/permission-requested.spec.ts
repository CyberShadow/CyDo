...
test("copilot built-in tool with permission auto-approval", async ({ page, agentType }) => {
  test.skip(agentType !== "copilot", "copilot-only test");

  // Create a file for the view tool to read.
  writeFileSync("/tmp/cydo-test-workspace/perm-test.txt", "permission-test-content\n");

  await enterSession(page);
  await sendMessage(page, "use builtin view /tmp/cydo-test-workspace/perm-test.txt");
  ...
  await expect(
    page.locator(".tool-name", { hasText: "view" }),
