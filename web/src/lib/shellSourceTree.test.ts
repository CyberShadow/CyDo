...
describe("sourceTree projection parser", () => {
  ...
  it("parses mixed quoted wrapper payload words and preserves raw source with decoded heredoc body", () => {
    ...
    expect(wrapper.origin?.attributes?.quote).toBe("projected");
  ...
  it("accepts adjacent quoted wrapper fragments as one payload word", () => {
    const parsed = parseShellCommandSourceTree("zsh -lc 'cat README.md''x'");
    expect(parsed.ok).toBe(true);
    if (!parsed.ok) return;
    const wrapper = findFirstEmbed(parsed.value.segments);
    expect(wrapper?.kind).toBe("embed");
  ...
  it("parses shell/bash heredoc bodies recursively and keeps markdown/xml heredocs as text nodes", () => {
    ...
    const directBashParsed = parseShellCommandSourceTree(directBashCommand);
    ...
    const shellParsed = parseShellCommandSourceTree(shellCommand);
    ...
    const markdownParsed = parseShellCommandSourceTree(markdownCommand);
    ...
    const svgParsed = parseShellCommandSourceTree(svgCommand);
  ...
  it("returns null (not exceptions) for invalid offsets and malformed projections", () => {
    ...
    const parsed = parseShellCommandSourceTree("zsh -lc 'cat README.md'");
    expect(parsed.ok).toBe(true);
    if (!parsed.ok) return;
    const wrapper = findFirstEmbed(parsed.value.segments);
    expect(wrapper).not.toBeNull();
  ...
  it("supports zero-length wrapper payload projections", () => {
    const parsed = parseShellCommandSourceTree('zsh -lc ""');
    expect(parsed.ok).toBe(true);
    if (!parsed.ok) return;
